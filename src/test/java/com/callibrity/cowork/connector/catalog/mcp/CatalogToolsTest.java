 ...
 * "given a mocked service, the right method is called with the right arguments and the return
 * value passes through unchanged." Business-logic coverage lives in {@code
 * DefaultCatalogServiceTest}.
 */
...
class CatalogToolsTest {
    ...
    void servicesListForwardsAllFilterAndPageArgs() {
        ...
        when(catalog.listServices("checkout", "pii", LifecycleStage.ACTIVE, 2, 50)).thenReturn(expected);
    ...
    private static ServiceDto stubServiceDto(String name) {
        return new ServiceDto(name, name, "", "platform", null, LifecycleStage.ACTIVE,
                null, null, null, 0, 0);
    }
