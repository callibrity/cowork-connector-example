package com.callibrity.cowork.connector.catalog.seed;

import static com.callibrity.cowork.connector.catalog.domain.DependencyType.CALLS;
import static com.callibrity.cowork.connector.catalog.domain.DependencyType.CONSUMES_FROM;
import static com.callibrity.cowork.connector.catalog.domain.DependencyType.PUBLISHES_TO;
import static com.callibrity.cowork.connector.catalog.domain.DependencyType.READS_FROM;
import static com.callibrity.cowork.connector.catalog.domain.LifecycleStage.ACTIVE;
import static com.callibrity.cowork.connector.catalog.domain.LifecycleStage.DEPRECATED;
import static com.callibrity.cowork.connector.catalog.domain.LifecycleStage.RETIRING;
...
public class CatalogSeeder implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        seedServices();
        seedDependencies();
        log.info("Seeded catalog: {} teams, {} services, {} dependencies.",
                teams.size(), services.size(), dependencyRepo.count());
    }
    ...
    private void seedTeams() {
        ...
        team("fulfillment", "Fulfillment & Logistics", "pagerduty://fulfillment-oncall", "#eng-fulfillment");
        team("data", "Data Platform", "pagerduty://data-oncall", "#eng-data");
        team("notifications", "Customer Messaging", "pagerduty://notifications-oncall", "#eng-notifications");
        team("integrations", "Partner Integrations", "pagerduty://integrations-oncall", "#eng-integrations");
    }
    ...
    private void seedServices() {
        ...
        svc("auth-service", "Auth Service",
                ...
                "platform", "platform", ACTIVE, tags("pii", "customer-facing", "foundation"));
        svc("feature-flags", "Feature Flags",
                "Runtime flag evaluation with percentage rollouts and per-tenant overrides.",
                "platform", "platform", ACTIVE, tags("foundation"));
        svc("kafka-gateway", "Kafka Gateway",
                "Internal Kafka client with schema registry enforcement.",
                "platform", "platform", ACTIVE, tags("foundation"));
        svc("log-aggregator", "Log Aggregator",
                "Ships structured logs from every service to the SIEM.",
                "platform", "platform", ACTIVE, tags("soc2-scope", "foundation"));
        svc("secrets-manager", "Secrets Manager",
                "Central secrets store with audit trail.",
                "platform", "platform", ACTIVE, tags("soc2-scope", "foundation"));

        // identity
        svc("accounts-api", "Accounts API",
                "Customer account CRUD including profile data.",
                "identity", "identity", ACTIVE, tags("pii", "customer-facing", "soc2-scope", "gdpr"));
        svc("sso-broker", "SSO Broker",
                "OIDC/SAML broker for partner-federated logins.",
                "identity", "identity", ACTIVE, tags("pii", "soc2-scope"));
        svc("session-store", "Session Store",
                "Redis-backed session persistence.",
                "identity", "identity", ACTIVE, tags("pii"));
        svc("password-reset", "Password Reset",
                "Self-service password reset flow.",
                "identity", "identity", ACTIVE, tags("pii", "customer-facing"));

        // checkout — concentrates PII and PCI
        svc("cart-service", "Cart Service",
                "Session-scoped shopping cart.",
                "checkout", "checkout", ACTIVE, tags("pii", "customer-facing"));
        svc("payment-processor", "Payment Processor",
                "Tokenizes cards and forwards authorizations to the PSP.",
                "checkout", "checkout", ACTIVE, tags("pii", "pci", "soc2-scope", "customer-facing"));
        svc("order-coordinator", "Order Coordinator",
                "Orchestrates cart -> payment -> fulfillment.",
                "checkout", "checkout", ACTIVE, tags("customer-facing"));
        svc("tax-calculator", "Tax Calculator",
                "Computes sales tax per SKU and destination.",
                "checkout", "checkout", ACTIVE, tags());
        svc("checkout-ui", "Checkout UI",
                "Server-side-rendered checkout pages.",
                "checkout", "checkout", ACTIVE, tags("customer-facing"));
        svc("cart-v1", "Cart Service (v1)",
                "Predecessor to cart-service. Kept alive for the mobile v4 client.",
                "checkout", "checkout", DEPRECATED, tags("pii"));

        // catalog
        svc("products-api", "Products API",
                "Read API for product metadata, pricing, and availability.",
                "catalog", "catalog", ACTIVE, tags("customer-facing"));
        svc("search-indexer", "Search Indexer",
                "Maintains Elasticsearch indexes from product mutations.",
                "catalog", "catalog", ACTIVE, tags());
        svc("inventory-service", "Inventory Service",
                "Real-time stock levels across warehouses.",
                "catalog", "catalog", ACTIVE, tags());
        svc("catalog-admin", "Catalog Admin",
                "Internal tool for merchandising and price updates.",
                "catalog", "catalog", ACTIVE, tags("soc2-scope"));

        // fulfillment
        svc("shipping-api", "Shipping API",
                "Orchestrates outbound shipments across carriers.",
                "fulfillment", "fulfillment", ACTIVE, tags("pii", "customer-facing"));
        svc("label-generator", "Label Generator",
                "Produces carrier-compliant shipping labels.",
                "fulfillment", "fulfillment", ACTIVE, tags("pii"));
        svc("returns-processor", "Returns Processor",
                "Refund and restock workflow for returned goods.",
                "fulfillment", "fulfillment", ACTIVE, tags("customer-facing"));
        svc("carrier-integration", "Carrier Integration",
                "Adapter layer for FedEx, UPS, and DHL APIs.",
                "fulfillment", "fulfillment", ACTIVE, tags("pii"));

        // data — has the naming-drift cluster
        svc("analytics-ingester", "Analytics Ingester",
                "Consumes events-bus and fans out to warehouse sinks.",
                "data", "data", ACTIVE, tags());
        svc("events-bus", "Events Bus",
                "Kafka-backed domain event topics.",
                "data", "data", ACTIVE, tags("foundation"));
        svc("reports-v2", "Reports v2",
                "The current reporting layer used by catalog-admin.",
                "data", "data", ACTIVE, tags());
        svc("reports-v2-new", "Reports v2 (New)",
                "The replacement for reports-v2. Rollout was paused in Q3.",
                "data", "data", ACTIVE, tags());
        svc("reporting-legacy", "Reporting (Legacy)",
                "Original reporting service. Scheduled for removal but still holds the tax summary endpoint.",
                "data", "data", DEPRECATED, tags("soc2-scope"));
        svc("etl-orchestrator", "ETL Orchestrator",
                "Airflow-equivalent batch scheduler.",
                "data", "data", ACTIVE, tags());

        // notifications
        svc("email-dispatcher", "Email Dispatcher",
                "Transactional email via the ESP.",
                "notifications", "notifications", ACTIVE, tags("pii", "customer-facing"));
        svc("push-dispatcher", "Push Dispatcher",
                "APNs / FCM fan-out.",
                "notifications", "notifications", ACTIVE, tags("customer-facing"));
        svc("sms-dispatcher", "SMS Dispatcher",
                "Transactional SMS via Twilio.",
                "notifications", "notifications", ACTIVE, tags("pii", "customer-facing"));
        svc("template-renderer", "Template Renderer",
                "Mustache rendering for notification bodies.",
                "notifications", "notifications", ACTIVE, tags());

        // integrations
        svc("partner-gateway", "Partner Gateway",
                "Inbound API for partner B2B integrations.",
                "integrations", "integrations", ACTIVE, tags("customer-facing"));
        svc("webhook-relay", "Webhook Relay",
                "Retries and dead-letters outbound webhook deliveries.",
                "integrations", "integrations", RETIRING, tags());

        // orphan — no owner, PCI-scoped, still in use
        svcOrphan("legacy-invoicing", "Legacy Invoicing",
                "Pre-Meridian invoicing system. Owner left in 2023; team was never reassigned.",
                "finance", DEPRECATED, tags("pci", "soc2-scope"));
    }

    private void seedDependencies() {
        // foundational fan-in — nearly everyone calls auth-service
        String[] authCallers = {
                "accounts-api", "sso-broker", "password-reset", "cart-service", "payment-processor",
                "order-coordinator", "checkout-ui", "products-api", "catalog-admin", "shipping-api",
                "returns-processor", "email-dispatcher", "push-dispatcher", "sms-dispatcher",
                "partner-gateway", "webhook-relay"
        };
        for (String caller : authCallers) dep(caller, "auth-service", CALLS);

        // feature-flags is consulted widely
        for (String caller : new String[]{"checkout-ui", "order-coordinator", "products-api",
                "search-indexer", "email-dispatcher", "partner-gateway", "catalog-admin"}) {
            dep(caller, "feature-flags", CALLS);
        }

        // kafka-gateway publishers
        for (String pub : new String[]{"order-coordinator", "payment-processor", "inventory-service",
                "shipping-api", "returns-processor", "accounts-api", "partner-gateway"}) {
            dep(pub, "kafka-gateway", PUBLISHES_TO);
        }

        // everyone logs
        for (String caller : new String[]{"auth-service", "accounts-api", "sso-broker", "payment-processor",
                "order-coordinator", "shipping-api", "partner-gateway", "webhook-relay"}) {
        ...
        // (checkout-ui uses cart-service, but cart-v1 lingers for the mobile v4 client via partner-gateway)
    ...
    private void svc(String name, String displayName, String description, String domain,
                     String ownerName, LifecycleStage lifecycle, Set<String> tags) {
        Team owner = teams.get(ownerName);
        if (owner == null) {
            throw new IllegalStateException("Unknown team: " + ownerName);
        }
        services.put(name, serviceRepo.save(new Service(
                name, displayName, description, domain, owner, lifecycle,
                "https://github.com/meridian/" + name,
                "https://runbooks.meridian.internal/" + name,
                tags)));
    }

    private void svcOrphan(String name, String displayName, String description, String domain,
                           LifecycleStage lifecycle, Set<String> tags) {
        services.put(name, serviceRepo.save(new Service(
                name, displayName, description, domain, null, lifecycle,
                "https://github.com/meridian/" + name,
                "https://runbooks.meridian.internal/" + name,
                tags)));
    }
